...
import com.icthh.xm.xmeplugin.ViewServer
import com.icthh.xm.xmeplugin.ViewServer.isDev
...
abstract class WebDialog(val project: Project,
                         viewName: String,
                         dimension: Dimension = Dimension(500, 500),
    ...
    override fun postponeValidation(): Boolean {
        // jCefWebPanelWrapper.browser?.openDevtools()
...
class JCefWebPanelWrapper(
    ...
    fun createCenterPanel(callbacks: (browser: JBCefBrowser) -> List<BrowserCallback>, onReady: (BrowserPipe) -> Unit = {}): JComponent {
        val url = "${ViewServer.serverUrl}/app?${pipeId}#/$viewName"
        val uiThreadAnchor = this.uiThreadAnchor

        val browser = JBCefBrowser()
        log.info("URL load ${url}")
...
class BrowserPipe(private val browser: JBCefBrowser, pipeId: String, viewName: String, callbacks: List<BrowserCallback>, onReady: (BrowserPipe) -> Unit = {}) : Disposable {
    ...
    init {
        ...
        val theme = if (JBColor.isBright()) "light-theme" else "dark-theme"
        val initCallback = """
    ...
    private fun addBrowserEvents(vararg names: String) {
        names.forEach {
            val browser: JBCefBrowserBase = browser
