...
import com.icthh.xm.xmeplugin.utils.getLinkedConfigRootDir
import com.icthh.xm.xmeplugin.utils.isXmeMicroservice
...
const val COMMONS_SUFFIX = ".groovy"
            ...
            PlatformPatterns.psiElement(), object : CompletionProvider<CompletionParameters>() {
                public override fun addCompletions(
                    ...
                    if (lepCommons != null && parent != null && parent.text.startsWith(commons)) {
                        ...
                        if (commonsPath.endsWith(DUMMY_IDENTIFIER_TRIMMED.replaceFirstChar { it.lowercase() })) {
                            commonsPath = commonsPath.substringBefore(DUMMY_IDENTIFIER_TRIMMED.replaceFirstChar {
                                it.lowercase()
                            })
                        }
...
public fun getVariants(commonsParts: List<String>, lepCommons: VirtualFile): List<String> {
    ...
    val variantMethods = folder.children.filter { !it.isDirectory }
        ...
        .filter { it.name.endsWith(COMMONS_SUFFIX_AROUND) || it.name.endsWith(COMMONS_SUFFIX_TENANT) || it.name.endsWith(COMMONS_SUFFIX) }
        .map { it.name }
...
public fun getLepFolder(project: Project, place: PsiElement): VirtualFile? {
    ...
    val pathToLepFolder = if(project.isXmeMicroservice()) {
        listOf("lep", "tenant", "microservice", "lep")
    } else {
        listOf("config", "tenants", "tenant", "microservice", "lep")
    }
